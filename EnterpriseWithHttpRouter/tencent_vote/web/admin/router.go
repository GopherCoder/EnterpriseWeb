...
func (a Admin) Register(writer http.ResponseWriter, request *http.Request) {
	...
	if err := json.NewDecoder(request.Body).Decode(&param); err != nil {
		make_response.Result(writer, request, http.StatusBadRequest, fmt.Errorf("bind error fail"))
		return
	}
	if err := param.Valid(); err != nil {
		make_response.Result(writer, request, http.StatusBadRequest, fmt.Errorf("invaild param fail"))
		return
	}
	log.Println("Param: ", param)
	var admin model.Admin
	if dbError := database.Engine.Where("phone = ?", param.Data.Phone).First(&admin).Error; dbError == nil {
		make_response.Result(writer, request, http.StatusBadRequest, fmt.Errorf("record exists"))
		return
	}
...
func (a Admin) Login(writer http.ResponseWriter, request *http.Request) {
	var param RegisterParams
	if err := json.NewDecoder(request.Body).Decode(&param); err != nil {
		make_response.Result(writer, request, http.StatusBadRequest, err)
		return
	}
	log.Println("Param: ", param)
	if err := param.Valid(); err != nil {
	...
	var admin model.Admin
	if dbError := database.Engine.Where("phone = ?", param.Data.Phone).First(&admin).Error; dbError != nil {
		make_response.Result(writer, request, http.StatusBadRequest, dbError)
		return
	}
	if ok := compareHashAndPassword([]byte(admin.Password), []byte(param.Data.Password)); !ok {
		make_response.Result(writer, request, http.StatusBadRequest, fmt.Errorf("password fail"))
		return
	}
	make_response.Result(writer, request, http.StatusOK, admin.Serializer())
...
func (a Admin) Logout(writer http.ResponseWriter, request *http.Request) {
	http.Redirect(writer, request, "/login", http.StatusOK)
}
